...
// NEW: Invite state tracking events
pub const KIND_GROUP_INVITE_DECLINE_9023: Kind = Kind::Custom(9023); // User -> Relay: Decline an invite
pub const KIND_GROUP_INVITE_SEEN_9024: Kind = Kind::Custom(9024);    // User -> Relay: Mark invite as seen
pub const KIND_GROUP_INVITE_DELETE_9025: Kind = Kind::Custom(9025);  // User -> Relay: Delete invite notification
...
pub const ALL_GROUP_KINDS_EXCEPT_DELETE_AND_ADDRESSABLE: [Kind; 13] = [
...
/// Status of an invite for a specific user
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum InviteStatus {
    Pending,  // Not acted upon
    Seen,     // Viewed but no action taken
    Accepted, // User joined using this invite
    Declined, // User declined the invite
    Deleted,  // User deleted the notification
}
...
pub struct Invite {
    ...
    // NEW: Invite state tracking per user
    pub declined_by: HashSet<(PublicKey, Timestamp)>,
    pub seen_by: HashSet<(PublicKey, Timestamp)>,
    pub deleted_by: HashSet<(PublicKey, Timestamp)>,
}
...
impl Invite {
    ...
    /// Mark invite as declined by a user
    pub fn mark_declined(&mut self, pubkey: PublicKey, timestamp: Timestamp) {
        self.declined_by.insert((pubkey, timestamp));
    }

    /// Mark invite as seen by a user
    pub fn mark_seen(&mut self, pubkey: PublicKey, timestamp: Timestamp) {
        self.seen_by.insert((pubkey, timestamp));
    }

    /// Mark invite as deleted by a user
    pub fn mark_deleted(&mut self, pubkey: PublicKey, timestamp: Timestamp) {
        self.deleted_by.insert((pubkey, timestamp));
    }
...
impl Group {
    ...
    /// Handle invite decline event
    pub fn handle_invite_decline(
        &mut self,
        event: Box<Event>,
        _relay_pubkey: &PublicKey,
    ) -> Result<Vec<StoreCommand>, Error> {
        if event.kind != KIND_GROUP_INVITE_DECLINE_9023 {
            return Err(Error::notice("Invalid event kind for invite decline"));
        }
        ...
        let invite_code = event
            ...
            .find(TagKind::custom("code"))
            .and_then(|t| t.content())
            .ok_or_else(|| Error::notice("No invite code found in decline event"))?;
    ...
    /// Handle invite seen event
    pub fn handle_invite_seen(
        &mut self,
        event: Box<Event>,
        _relay_pubkey: &PublicKey,
    ) -> Result<Vec<StoreCommand>, Error> {
        if event.kind != KIND_GROUP_INVITE_SEEN_9024 {
            return Err(Error::notice("Invalid event kind for invite seen"));
        }
        ...
        let invite_code = event
            ...
            .find(TagKind::custom("code"))
            .and_then(|t| t.content())
            .ok_or_else(|| Error::notice("No invite code found in seen event"))?;
    ...
    pub fn handle_invite_delete(
        ...
        if event.kind != KIND_GROUP_INVITE_DELETE_9025 {
            return Err(Error::notice("Invalid event kind for invite delete"));
        }
        ...
        let invite_code = event
            ...
            .find(TagKind::custom("code"))
            .and_then(|t| t.content())
            .ok_or_else(|| Error::notice("No invite code found in delete event"))?;
    ...
    /// Check if invite was declined by specific user
    pub fn is_declined_by(&self, pubkey: &PublicKey) -> bool {
        self.declined_by.iter().any(|(pk, _)| pk == pubkey)
    }

    /// Check if invite was seen by specific user
    pub fn is_seen_by(&self, pubkey: &PublicKey) -> bool {
        self.seen_by.iter().any(|(pk, _)| pk == pubkey)
    }

    /// Check if invite was deleted by specific user
    pub fn is_deleted_by(&self, pubkey: &PublicKey) -> bool {
        self.deleted_by.iter().any(|(pk, _)| pk == pubkey)
    }

    /// Get the overall status for a specific user
    pub fn get_status_for_user(&self, pubkey: &PublicKey) -> InviteStatus {
        if self.is_deleted_by(pubkey) {
            InviteStatus::Deleted
        } else if self.redeemed_by.as_ref().map_or(false, |(pk, _)| pk == pubkey) {
            InviteStatus::Accepted
        } else if self.is_declined_by(pubkey) {
            InviteStatus::Declined
        } else if self.is_seen_by(pubkey) {
            InviteStatus::Seen
        } else {
